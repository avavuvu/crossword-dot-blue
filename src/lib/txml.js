...
export function simplify(children) {
    ...
    children.forEach(function(child) {
        ...
        var kid = simplify(child.children);

        if(typeof kid !== 'string') {
            for(const [tag, value] of Object.entries(kid)) {
                if(value === "") {
                    kid[tag] = true
                }
            }
        ...
        out[child.tagName].push(kid);

        if (Object.keys(child.attributes).length && typeof kid !== 'string') {
